...
class Terrain(
    ...
    fun getHeightOfTerrain(worldX: Float, worldZ: Float): Float {
        ...
        return if (xCoordinate <= 1 - zCoordinate) {
            getHeightFromBarycentricCoordinates(
                Vector3D(0f, heights[gridX][gridZ], 0f),
                Vector3D( 1f, heights[gridX + 1][gridZ], 0f),
                Vector3D(0f, heights[gridX][gridZ + 1], 1f),
                Vector2D(xCoordinate, zCoordinate)
            )
        } else {
            getHeightFromBarycentricCoordinates(
                Vector3D(1f, heights[gridX + 1][gridZ], 0f),
                Vector3D(1f, heights[gridX + 1][gridZ + 1], 1f),
                Vector3D(0f, heights[gridX][gridZ + 1], 1f),
                Vector2D(xCoordinate, zCoordinate)
            )
        }
    ...
    private fun calculateNormal(x: Int, y: Int, image: BufferedImage):Vector3D {
        ...
        val normal = Vector3D(heightL - heightR, 2f, heightD - heightU)
        normal.normalize()
