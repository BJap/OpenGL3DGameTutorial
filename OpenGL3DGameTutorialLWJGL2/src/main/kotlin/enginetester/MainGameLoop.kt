...
fun main() {
    ...
    val backgroundTexture = TerrainTexture(loader.load2DTexture("src/main/resources/textures/grassy3.png"))
    ...
    entities.add(Entity(texturedLampModel, Vector3f(185f, -4.7f, -293f), 0f, 0f, 0f, 1f))
    entities.add(Entity(texturedLampModel, Vector3f(370f, 4.2f, -300f), 0f, 0f, 0f, 1f))
    entities.add(Entity(texturedLampModel, Vector3f(293f, -6.8f, -305f), 0f, 0f, 0f, 1f))
    ...
    val player = Player(texturedPlayerModel, Vector3f(400f, 5f, -150f), 0f, 180f, 0f, 0.3f)

    entities.add(player)

    val healthTextureId = loader.load2DTexture("src/main/resources/textures/health.png")
    val healthGuiTexture = GuiTexture(healthTextureId, Vector2f(-0.6f, -0.8f), Vector2f(0.3f, 0.4f))

    val guis = ArrayList<GuiTexture>()
    guis.add(healthGuiTexture)

    val sun = Light(Vector3f(0f, 1000f, -7000f), Vector3f(0.4f, 0.4f, 0.4f))
    val redLight = Light(Vector3f(185f, 10f, -293f), Vector3f(2f, 0f, 0f), Vector3f(1f, 0.01f, 0.002f))
    val yellowLight = Light(Vector3f(370f, 17f, -300f), Vector3f(0f, 2f, 2f), Vector3f(1f, 0.01f, 0.002f))
    val greenLight = Light(Vector3f(293f, 7f, -305f), Vector3f(2f, 2f, 0f), Vector3f(1f, 0.01f, 0.002f))
